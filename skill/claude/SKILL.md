...
| Command | Argument | Reference file | Output |
|---|---|---|---|
| `/sage spec <description>` | Feature description, ticket text, or idea | `references/spec.md` | Interview → spec.md + behavior.md |
| `/sage behavior` | *(none — reads spec.md)* | `references/behavior.md` | behavior.md (standalone regeneration) |
| `/sage code` | *(none — reads spec.md + behavior.md)* | `references/code.md` | Implementation files |
| `/sage update <feedback>` | Precise feedback tied to spec or behavior | `references/update.md` | Updated files |
| `/sage pr` | *(none — reads full context)* | `references/pr.md` | pr.md |
| `/sage doc` | *(none — reads full context)* | `references/doc.md` | doc.md |
| `/sage harness init` | *(none — interactive wizard)* | `references/harness-init.md` | .sage/harness.md |
| `/sage harness generate` | *(none — reads codebase files)* | `references/harness-generate.md` | .sage/harness.md |
| `/sage harness review` | *(none — reads harness.md)* | `references/harness-review.md` | Review report |
...
### Special flow for `/sage spec`

`/sage spec` is a two-phase command — it does NOT generate files immediately:

1. **Interview phase** — analyze the description, prepare questions, show ALL questions at once with a count ("I have X questions"), wait for answers
2. **Generation phase** — after answers received, generate `spec.md` then `behavior.md` in the same response

Never skip the interview. Even if the description seems complete, ask at minimum 3 questions to confirm acceptance criteria and restrictions.
